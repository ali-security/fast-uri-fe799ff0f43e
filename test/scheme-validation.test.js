...
test('decoded schemes select their scheme handlers', (t) => {
  ...
  // This version registers no mailto handler, so the ws handler stands in: it
  // only runs when the *decoded* scheme selects it, which is exactly what a
  // percent-encoded scheme used to hide.
  const ws = fastURI.parse('w%73://example.com/chat')
  t.equal(ws.scheme, 'ws', 'decoded scheme replaces the encoded one')
  t.equal(ws.resourceName, '/chat', 'ws parsing runs after decoding the scheme')
  t.equal(ws.secure, false, 'ws parsing runs after decoding the scheme')
  t.equal(
    fastURI.normalize('w%73://example.com:80/chat'),
    'ws://example.com/chat',
    'ws normalization runs after decoding the scheme'
  )
  t.end()
})
