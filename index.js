...
function parseWithStatus (uri, opts) {
  ...
  if (matches) {
    ...
    // Decode and validate the scheme before anything else consumes it: the
    // scheme selects the scheme handler and is echoed back by serialize(), so a
    // percent-encoded scheme that only decodes to "//evil.example" or
    // "\r\nSet-Cookie" once normalization runs would otherwise smuggle an
    // authority, a control character, or a different scheme past every check.
    ...
    if (!options.unicodeSupport && (!schemeHandler || !schemeHandler.unicodeSupport)) {
      ...
      if (parsed.host && parsed.host[0] !== '[' && (options.domainHost || (schemeHandler && schemeHandler.domainHost)) && isIP === false && nonSimpleDomain(parsed.host)) {
...
function normalizeComparableURI (uri, opts) {
  ...
  if (typeof uri === 'object') {
    // serialize() rejects a component scheme that decodes to an invalid
    // identifier; comparison must fail closed rather than propagate the throw.
    try {
